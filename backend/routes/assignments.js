...
module.exports = function (db) {
  ...
  // Returns users NOT yet registered to this activity (for the dropdown)
  router.get('/available/:activity_id', requireAdmin, async (req, res) => {
    try {
      ...
      const users = await db.all(`
        ...
        WHERE u.is_admin = 0
          AND u.id NOT IN (
            SELECT r.user_id FROM registrations r WHERE r.activity_id = ?
          )
  ...
  // POST /api/assignments — admin assigns a user to an activity
  router.post('/', requireAdmin, async (req, res) => {
    try {
      ...
      if (already)
        return res.status(409).json({ error: `${user.username} כבר משובץ לפעילות זו` });

      // Overlap check — same logic as self-registration
        ...
        if (timesOverlap(activity.start_time, activity.end_time, existing.start_time, existing.end_time)) {
          if (!activity.allow_overlap && !existing.allow_overlap) {
            return res.status(409).json({
              error: `לא ניתן לשבץ את ${user.username} — חפיפה עם "${existing.title}" (${existing.start_time}–${existing.end_time}). אף פעילות לא מאפשרת חפיפה.`
            });
          }
        }
      }
  ...
  // DELETE /api/assignments/:activity_id/:user_id — admin removes a user
  router.delete('/:activity_id/:user_id', requireAdmin, async (req, res) => {
    try {
      ...
      if (!reg)
        return res.status(404).json({ error: 'עובד לא משובץ לפעילות זו' });
